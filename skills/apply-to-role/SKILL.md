...
anything is sent - and before any change to the resume's bullet wording or summary (see Step 4a).
...
3. **Classify growth stage** (Seed → Series A/B → C+/pre-IPO/public) using `positioning-brain.md` §2.
...
3. **Apollo** - primary contact + email (Apollo MCP if connected, else Apollo export).
   Title-target: Founder/CEO, Head of Growth, Head of Marketing, CMO.
4. **Apify** - a LinkedIn company-employees actor to locate by title; an email finder/verifier
   actor for the address.
...
- **Role signal (free, always):** what the job post tells you they care about, plus anything
  it links to (Step 1.2). Usually the richest, most specific material.
- **Product signal (free, web fetch):** a feature they recently shipped, a smart product
  decision, the problem they're solving and why it's interesting. Frame as admiration, e.g.
  "loved the new {feature} on {product}, because {reason}."
- **Company / press signal:** recent funding, a launch, a milestone, a notable customer. Recent
  press and raises are great openers ("congrats on the {round}").
- **Founder / person signal:** a recent LinkedIn or Twitter/X post or public take of theirs.
  Eddie has LinkedIn and Twitter/X post-scraper actors set up in Apify - use them to pull the
  founder's recent posts. Also look for shared background or common ground.
...
### 4a. Tailored resume
- Start from `master-resume.md`. Apply the **foreground table** in `positioning-brain.md` §3 for
  the role's lean (founding / CRO / PMM / content / performance / AI-growth): lead with the
  matching blocks, reorder, and select which bullets to include. Keep the pivot narrative spine intact.
- **For early-stage / seed roles, foreground the early-startup experience (Vandalsoft):** joining
  a startup early, helping raise seed, and launching real products is proof Eddie thrives in
  0-to-1 chaos. It's an asset here, not something to trim.
- **Always lead the Gro.X section with the $103K revenue / 15 B2B SaaS clients bullet** - it gives
  instant context. Tailor the order of the bullets that follow it per the role.
- **Freelance block title swap (role-driven):** use "UX Designer" for design / product / UX-leaning
  roles, and "Freelance CRO / UX Consultant" (or "UX Consultant") for growth / CRO / conversion roles.
  State which title you picked in the change proposal so Eddie can veto.
- Re-angle the intro line per the stage. Apply every rule in `tone-and-guardrails.md`
  (bold accomplishments not tools; no em-dashes; no summary/skills block). **Length follows the
  role; do not force a single page.**

**Change control - propose, then confirm, before altering any content (required):**
The master resume is the source of truth. You may freely reorder blocks and choose which bullets
to include, but you do not edit what the resume says on your own:
- **Never rewrite a bullet's wording yourself.** Use master bullets verbatim. Changing what a
  bullet *says* requires Eddie's explicit go-ahead.
- **Dropping bullets:** you may leave out low-relevance bullets to fit the role, but list every
  bullet you propose to cut and get Eddie's confirmation before rendering. Never silently remove one.
- **Better-fit wording:** if a bullet's content could be reworded to fit the role significantly
  better, *suggest* the specific edit to Eddie and wait for approval - do not apply it yourself.
- **Summary / intro line:** if it should change for this role, propose the new line to Eddie first;
  do not finalize a changed summary without approval.
- **Title swap:** allowed as a role-driven choice, but name which title you picked.

Present the proposed changes as a short list (bullets to drop, any suggested rewordings, summary
change, title choice), get Eddie's OK, then render. Reordering and selecting which master bullets
appear is fine to do directly; rewording is not.

- Render with the existing generators: `generate_ats_resume.js` (ATS .docx) and
  `build_designed_resume.py` (designed PDF). Output `Eddie_Park_Resume_{Company}.docx` / `.pdf`.

### 4b. Personalized intro line
One line, about them first, re-angled from the default in `positioning-brain.md` §4.

### 4c. Outreach message - guidelines first, template second

This is judgment, not a form. Write the message that a real, sharp candidate would send after
genuinely looking into the company. Hold every draft to these guidelines:
...
- **If you have a sharp, correct insight** (not criticism) that shows he speaks B2B SaaS, you can
  include it - only when it's genuine.
- **Offer the conversion audit only when the role and company stage make it natural** (early/mid
  stage where conversion is a live problem). Otherwise leave it out.
...
> I noticed you were looking for a {role}, and wanted to introduce myself.
> {situational line}
> Looking forward to your thoughts!

Situational lines (pick what fits, or write a better one - these are not the only options):
- **Conversion-audit offer (when stage fits):** "I previously built a B2B SaaS CRO agency, and
  I'd love to offer a quick conversion audit on turning more of your traffic into demos."
- **Connect / direction:** "Would love to connect and hear where you're looking to take
  {Company}."
- **Insight / common ground:** a genuine "loved your take on {industry shift}" or "loved {thing}
  about your background - we have that in common."

For a **no-open-role** prospect, drop the role line; lead with the signal and a light "wanted to
be on your radar as you scale."

### 4d. [Conditional] Bespoke outreach strategy - required when warranted

If the research surfaces a standout opening - a job post that references a person/video/idea, a
founder who clearly values a specific style, a recent moment worth a more creative move - you are
**required** to propose a tailored strategy beyond the standard message. Example: a Lassie growth
role that says "think of this role as Lassie's Matt Epstein" and links a video - review the video
(or ask Eddie for the transcript), understand that growth archetype, and build outreach that
speaks to it directly (and angle the resume to match). Spell out the angle, why it fits the
...
- Resume content: never rewrite bullet wording or the summary without Eddie's explicit approval; propose bullet drops, any rewordings, and the UX title choice, and get the OK before rendering. Reordering and selecting which master bullets appear is fine.
...
- Privacy/safety: don't send messages, submit applications, or email without explicit human
  action; flag any unverified email.
